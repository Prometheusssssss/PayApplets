...
Page({
  ...
  onLoad: function (options) {
    var that = this;
    if(options){
      var adventureInfo = JSON.parse(options.adventure);
      if(adventureInfo.CLUE != ''&& adventureInfo.CLUE != undefined && adventureInfo.CLUE != null){
        var uploaderList = [];
        if(adventureInfo.COORDINATE_URL != ''){
          if(adventureInfo.COORDINATE_URL!= undefined && adventureInfo.COORDINATE_URL!= null&& adventureInfo.COORDINATE_URL!= ''){
            uploaderList = adventureInfo.COORDINATE_URL.split(',');
 ...
 // 删除图片
  ...
  saveGuestValue: async function(e){
    ...
    var p = {
      ...
      CONTRIBUTOR: adventureInfo.CONTRIBUTOR == null?'': adventureInfo.CONTRIBUTOR,
      CONTRIBUTOR_ID: adventureInfo.CONTRIBUTOR_ID == null?'': adventureInfo.CONTRIBUTOR_ID,
      COORDINATE_URL: mainImgUrl
    }
    if(that.data.pageType == 'add'){
      p.KID = -1;
    }else  if(that.data.pageType == 'edit'){
      p.KID = adventureInfo.DETAIL_ID
    }
