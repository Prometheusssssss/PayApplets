...
Page({
  ...
  onLoad: function (options) {
    var that = this;
    if(options){
      var cookInfo = JSON.parse(options.adventure);
      var uploaderList = [];
      if(cookInfo.CONTENT != ''){
        if(cookInfo.CONTENT!= undefined && cookInfo.CONTENT!= null&& cookInfo.CONTENT!= ''){
          uploaderList = cookInfo.CONTENT.split(',');
 ...
 // 删除图片
  ...
  saveGuestValue: async function(e){
    ...
    var p = {
      ...
      CONTRIBUTOR: cookInfo.CONTRIBUTOR == null?'': cookInfo.CONTRIBUTOR,
      CONTRIBUTOR_ID: cookInfo.CONTRIBUTOR_ID == null? '': cookInfo.CONTRIBUTOR_ID,
    }
    if(that.data.pageType == 'add'){
      p.KID = -1;
    }else  if(that.data.pageType == 'edit'){
      p.KID = cookInfo.DETAIL_ID
    }
    
    that.createAdventure(p)
  },
  createAdventure: function (p) {
    app.ManageExecuteApi('/api/_cud/createAndUpdate/B_CAIPU', '', p, 'POST').then((result) => {
      wx.hideLoading()
      if (result != 'error') {
        wx.showToast({
          title: '保存成功',
  ...
  confirmClear:function(){
    var that = this;
    var p = {
      KID: that.data.cookInfo.DETAIL_ID,
      CONTRIBUTOR: '',
      CONTRIBUTOR_ID: '',
    }
    app.ManageExecuteApi('/api/_cud/createAndUpdate/b_adventure_strategy', '', p, 'POST').then((result) => {
      if (result != 'error') {
          ...
          that.setData({
            showClearConfirm:false,
            ["cookInfo.CONTRIBUTOR"] : '',
            ["cookInfo.CONTRIBUTOR_ID"] : '',
          })
      }
    })
  }
